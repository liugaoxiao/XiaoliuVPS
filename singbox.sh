...
# 每小时检查日志大小，超过 10MB 时归档；归档保留 14 天。
...
_setup_log_cleanup() {
    command -v crontab >/dev/null 2>&1 || {
        _warning "未找到 crontab，无法启用日志轮转定时任务。"
        return 1
    }
...
_update_script() {
    ...
    if wget -qO "$temp_script_path" "$main_script_url"; then
        ...
        if [ -z "$downloaded_version" ] || ! head -n 1 "$temp_script_path" | grep -q '^#!/bin/bash' || ! grep -q '_operations_menu' "$temp_script_path" || ! grep -q '21) _operations_menu' "$temp_script_path"; then
            _error "下载内容不是包含完整运维功能的 singbox.sh，已拒绝覆盖本地脚本。"
            rm -f "$temp_script_path"
            return 1
        fi
...
# 包含: sysctl 内核参数 + nftables MSS Clamp/DDoS + 文件描述符 + RPS/RFS
# ═══════════════════════════════════════════════════════════════

# sysctl 优化参数内容（写入 /etc/sysctl.conf）
SYSCTL_OPTIMIZE_CONTENT='
# ═══ VPS 网络优化 - 整合方案 ═══
# 来源: Wyatt TCP+BBR / vps-netpilot / KEJILION.SH
# 更新: 2026-08-06
...
_vps_optimize_legacy() {
    ...
    # 显示当前状态
    _info "当前系统信息:"
    echo -e "  系统: $(uname -s -r -m)"
    echo -e "  CPU:  $(nproc 2>/dev/null || echo '?') 核"
    echo -e "  内存: $(free -h 2>/dev/null | awk '/^Mem:/{print $2}' || echo '?')"
    local old_bbr=$(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null || echo "cubic")
    echo -e "  当前拥塞算法: ${old_bbr}"
    echo ""

    echo -e "${YELLOW}  即将执行以下优化:${NC}"
    echo -e "  1. 写入 sysctl 内核参数 (33项)"
    echo -e "  2. 加载 BBR/conntrack 模块"
    echo -e "  3. 设置开机自动加载模块"
    echo -e "  4. 配置文件描述符上限 (1048576)"
    echo -e "  5. 配置 IPv4 优先解析 (gai.conf)"
    echo -e "  6. nftables MSS Clamp 优化"
    echo -e "  7. nftables DDoS 防护规则"
    echo -e "  8. 验证优化结果"
    echo ""
    
    read -p "  确认执行优化? (Y/n): " confirm
    [[ "$confirm" == "n" || "$confirm" == "N" ]] && _info "已取消" && return 0
    echo ""

    # Step 1: 备份并写入 sysctl 参数
    _info "[1/7] 备份原始 sysctl.conf..."
    local backup_file="/etc/sysctl.conf.bak.$(date +%Y%m%d%H%M%S)"
    [ -f /etc/sysctl.conf ] && cp /etc/sysctl.conf "$backup_file" && _success "已备份到: $backup_file"
    
    _info "[1/7] 写入优化参数 (33项)..."
    echo "$SYSCTL_OPTIMIZE_CONTENT" > /etc/sysctl.conf
    sysctl -p >/dev/null 2>&1
    _success "sysctl 参数已生效"

    # Step 2: 加载内核模块
    _info "[2/7] 加载内核模块..."
    modprobe tcp_bbr 2>/dev/null && _success "tcp_bbr 模块已加载" || _warn "tcp_bbr 加载失败 (可能已内置)"
    modprobe nf_conntrack 2>/dev/null && _success "nf_conntrack 模块已加载" || _warn "nf_conntrack 加载失败"

    # Step 3: 开机自动加载
    _info "[3/7] 设置开机自动加载模块..."
...
EOF
        _success "文件描述符上限已设为 1048576"
    else
        _success "文件描述符已配置，跳过"
    fi

    # Step 5: IPv4 优先解析
    _info "[5/7] 配置 IPv4 优先解析..."
    mkdir -p /etc
    if [ -f /etc/gai.conf ]; then
        if ! grep -q 'prefer_ipv4' /etc/gai.conf 2>/dev/null; then
            echo 'precedence ::ffff:0:0/96  100' >> /etc/gai.conf
            _success "已添加 IPv4 优先规则"
        else
            _success "IPv4 优先已配置，跳过"
        fi
    else
        echo '# VPS-Optimize: IPv4 优先解析
precedence ::ffff:0:0/96  100' > /etc/gai.conf
        _success "已创建 gai.conf 并配置 IPv4 优先"
    fi

    # Step 6: nftables MSS Clamp
    _info "[6/7] 配置 nftables MSS Clamp..."
    if command -v nft &>/dev/null; then
        # 创建独立的优化表 (不污染 singboxlite 表)
        nft list table inet vps_optimize >/dev/null 2>&1 || nft add table inet vps_optimize
        nft list chain inet vps_optimize mangle_forward >/dev/null 2>&1 || \
            nft add chain inet vps_optimize mangle_forward '{ type filter hook forward priority -150; policy accept; }'
        # 清除旧规则再添加
        ...
        if [ -f /etc/nftables.conf ] && ! grep -q 'vps_optimize\.nft' /etc/nftables.conf 2>/dev/null; then
            echo 'include "/etc/nftables.d/vps_optimize.nft"' >> /etc/nftables.conf
        fi
        _success "MSS Clamp 规则已生效"
    else
        _warn "nftables 不可用，跳过 MSS Clamp"
    fi

    # Step 7: DDoS 防护
    _info "[7/7] 配置 DDoS 防护规则..."
    if command -v nft &>/dev/null; then
        nft list table inet vps_optimize >/dev/null 2>&1 || nft add table inet vps_optimize
        # INPUT 链限速
        nft list chain inet vps_optimize dos_input >/dev/null 2>&1 || \
            nft add chain inet vps_optimize dos_input '{ type filter hook input priority -150; policy accept; }'
        ...
        # 持久化
        nft list table inet vps_optimize > /etc/nftables.d/vps_optimize.nft 2>/dev/null
        _success "DDoS 防护规则已生效"
    else
        _warn "nftables 不可用，跳过 DDoS 防护"
    fi
    ...
    local new_bbr=$(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null)
    ...
    local retries2=$(sysctl -n net.ipv4.tcp_retries2 2>/dev/null)
    local swappiness=$(sysctl -n vm.swappiness 2>/dev/null)
    local conntrack_max=$(sysctl -n net.netfilter.nf_conntrack_max 2>/dev/null)
    
    echo -e "  拥塞算法: ${old_bbr} → ${GREEN}${new_bbr}${NC}"
    echo -e "  队列调度: ${GREEN}${qdisc}${NC}"
    echo -e "  连接队列: ${GREEN}${somaxconn}${NC}"
    echo -e "  接收缓冲: ${GREEN}$((rmem_max / 1048576)) MB${NC}"
    echo -e "  文件句柄: ${GREEN}${file_max}${NC}"
    echo -e "  fin_timeout: ${GREEN}${fin_timeout}s${NC}"
    echo -e "  tcp_retries2: ${GREEN}${retries2}${NC}"
    echo -e "  swappiness: ${GREEN}${swappiness}${NC}"
    echo -e "  conntrack_max: ${GREEN}${conntrack_max}${NC}"
    echo ""
    echo -e "  ${GREEN}✓ BBR + FQ 拥塞控制${NC}"
    echo -e "  ${GREEN}✓ TCP/UDP 缓冲区 64MB${NC}"
    echo -e "  ${GREEN}✓ IPv4/IPv6 转发已开启${NC}"
    echo -e "  ${GREEN}✓ TCP Fast Open 已启用${NC}"
    echo -e "  ${GREEN}✓ 文件描述符 1048576${NC}"
    echo -e "  ${GREEN}✓ IPv4 优先解析${NC}"
    echo -e "  ${GREEN}✓ MSS Clamp (nftables)${NC}"
    echo -e "  ${GREEN}✓ DDoS 防护 (nftables)${NC}"
    echo ""
    echo -e "${YELLOW}══════════════════════════════════════════════════════${NC}"
}

# 菜单 20 使用仓库中独立维护的优化脚本，避免旧内嵌逻辑覆盖系统配置或隐藏 sysctl 错误。
_vps_optimize() {
    local url="${GITHUB_RAW_BASE}/optimize.sh"
    local tmp="/tmp/vps-optimize.$$.sh"
    clear
    _info "正在从 XiaoliuVPS 下载最新优化脚本..."
    if command -v curl >/dev/null 2>&1; then
        curl -fL --retry 2 --connect-timeout 10 -o "$tmp" "${url}?v=$(date +%s)"
    elif command -v wget >/dev/null 2>&1; then
        wget -qO "$tmp" "${url}?v=$(date +%s)"
    else
        _error "需要 curl 或 wget 才能下载优化脚本。"
        return 1
    fi
    if [ ! -s "$tmp" ] || ! head -n 1 "$tmp" | grep -q '^#!/.*bash' || ! bash -n "$tmp"; then
        rm -f "$tmp"
        _error "优化脚本下载或语法校验失败，未执行任何优化。"
        return 1
    fi
    chmod 700 "$tmp"
    bash "$tmp"
...
LOG_ARCHIVE_DIR="${SINGBOX_DIR}/logs/archive"
...
_service_state_label() {
    ...
    if [ "$INIT_SYSTEM" = "systemd" ]; then
        systemctl is-active --quiet "$service_name" 2>/dev/null && printf '运行中' || printf '已停止'
    elif [ "$INIT_SYSTEM" = "openrc" ]; then
        rc-service "$service_name" status 2>/dev/null | grep -q 'started' && printf '运行中' || printf '已停止'
    else
        printf '未知'
    fi
}

_show_operations_status() {
    local bbr qdisc mem_summary disk_summary load_avg sb_count xray_count nft_state
    clear
    echo -e "${CYAN}╔══════════════ VPS 运维状态 ══════════════╗${NC}"
    echo ""
    bbr=$(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null || echo '不可用')
    qdisc=$(sysctl -n net.core.default_qdisc 2>/dev/null || echo '不可用')
    mem_summary=$(free -h 2>/dev/null | awk '/^Mem:/ {print $3 "/" $2}' || echo '不可用')
    disk_summary=$(df -hP / 2>/dev/null | awk 'NR==2 {print $3 "/" $2 " (" $5 ")"}' || echo '不可用')
    load_avg=$(cut -d' ' -f1-3 /proc/loadavg 2>/dev/null || echo '不可用')
    sb_count=$(jq '[.inbounds[]? | select(.tag | startswith("argo-") | not)] | length' "$CONFIG_FILE" 2>/dev/null || echo 0)
    xray_count=$(jq '.inbounds | length' /usr/local/etc/xray/config.json 2>/dev/null || echo 0)
    nft list table inet vps_optimize >/dev/null 2>&1 && nft_state='已加载' || nft_state='未加载/未安装'

    echo -e "  系统运行时间: ${GREEN}$(uptime -p 2>/dev/null || uptime)${NC}"
    echo -e "  系统负载:     ${GREEN}${load_avg}${NC}"
    echo -e "  内存已用:     ${GREEN}${mem_summary}${NC}"
    echo -e "  根分区已用:   ${GREEN}${disk_summary}${NC}"
    echo ""
    echo -e "  Sing-box:     ${GREEN}$(_service_state_label sing-box)${NC} (${sb_count} 个节点)"
    echo -e "  Xray:         ${GREEN}$(_service_state_label xray)${NC} (${xray_count} 个节点)"
    echo -e "  网络优化:     拥塞控制 ${GREEN}${bbr}${NC}，队列 ${GREEN}${qdisc}${NC}"
    echo -e "  防火墙表:     ${GREEN}${nft_state}${NC}"
    echo ""
    echo -e "  ${CYAN}监听端口（前 20 项）${NC}"
    if command -v ss >/dev/null 2>&1; then
        ss -lntup 2>/dev/null | sed -n '1,21p'
    else
        echo "  未安装 ss，无法列出监听端口。"
    fi
    echo ""
    echo -e "  配置快照: ${GREEN}$(find "$BACKUP_DIR" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')${NC} 个"
    echo -e "  日志归档: ${GREEN}$(find "$LOG_ARCHIVE_DIR" -type f 2>/dev/null | wc -l | tr -d ' ')${NC} 个"
    echo ""
...
_create_config_backup() {
    local reason="${1:-manual}" timestamp snapshot
    timestamp=$(date +%Y%m%d_%H%M%S)
    snapshot="${BACKUP_DIR}/${timestamp}_${reason}"
    mkdir -p "$snapshot"
    _backup_copy_file "$CONFIG_FILE" "$snapshot/sing-box/config.json"
    _backup_copy_file "${SINGBOX_DIR}/relay.json" "$snapshot/sing-box/relay.json"
    _backup_copy_file "$METADATA_FILE" "$snapshot/sing-box/argo.json"
    _backup_copy_file "$CLASH_YAML_FILE" "$snapshot/sing-box/clash.yaml"
    _backup_copy_file "/usr/local/etc/xray/config.json" "$snapshot/xray/config.json"
    printf 'created_at=%s\nreason=%s\n' "$(date -Is)" "$reason" > "$snapshot/manifest"
    local snapshots=() old_snapshot
    mapfile -t snapshots < <(find "$BACKUP_DIR" -mindepth 1 -maxdepth 1 -type d | sort -r)
    for old_snapshot in "${snapshots[@]:10}"; do
        rm -rf "$old_snapshot"
    done
    _success "配置快照已创建：${snapshot}"
}
...
_restore_config_backup() {
    local backups=() selected snapshot
    mapfile -t backups < <(_list_config_backups)
    if [ "${#backups[@]}" -eq 0 ]; then
        _warning "暂无可恢复的配置快照。"
        return 0
    fi
    echo -e "${CYAN}可恢复的配置快照：${NC}"
    local i=1
    for selected in "${backups[@]}"; do
        echo "  [${i}] ${selected}"
        i=$((i + 1))
    done
    read -r -p "请选择快照编号（0 返回）: " selected
    [[ "$selected" =~ ^[0-9]+$ ]] || { _error "请输入数字。"; return 1; }
    [ "$selected" -eq 0 ] && return 0
    [ "$selected" -le "${#backups[@]}" ] || { _error "编号超出范围。"; return 1; }
    snapshot="${BACKUP_DIR}/${backups[$((selected - 1))]}"
    read -r -p "恢复 ${backups[$((selected - 1))]}？当前配置会先自动备份。 (y/N): " confirm
    [[ "$confirm" =~ ^[Yy]$ ]] || { _info "已取消恢复。"; return 0; }
    _create_config_backup pre_restore
    _backup_copy_file "$snapshot/sing-box/config.json" "${CONFIG_FILE}.restore.tmp"
    _backup_copy_file "$snapshot/sing-box/relay.json" "${SINGBOX_DIR}/relay.json.restore.tmp"
    _backup_copy_file "$snapshot/sing-box/argo.json" "${METADATA_FILE}.restore.tmp"
    _backup_copy_file "$snapshot/sing-box/clash.yaml" "${CLASH_YAML_FILE}.restore.tmp"
    if [ -f "${CONFIG_FILE}.restore.tmp" ] && [ -x "$SINGBOX_BIN" ] && ! "$SINGBOX_BIN" check -c "${CONFIG_FILE}.restore.tmp" -c "${SINGBOX_DIR}/relay.json.restore.tmp" >/dev/null 2>&1; then
        rm -f "${CONFIG_FILE}.restore.tmp" "${SINGBOX_DIR}/relay.json.restore.tmp" "${METADATA_FILE}.restore.tmp" "${CLASH_YAML_FILE}.restore.tmp"
        _error "快照未通过 sing-box 配置校验，已取消恢复。"
        return 1
    fi
    if [ -f "$snapshot/xray/config.json" ] && [ -x /usr/local/bin/xray ] && ! /usr/local/bin/xray run -test -config "$snapshot/xray/config.json" >/dev/null 2>&1; then
        rm -f "${CONFIG_FILE}.restore.tmp" "${SINGBOX_DIR}/relay.json.restore.tmp" "${METADATA_FILE}.restore.tmp" "${CLASH_YAML_FILE}.restore.tmp"
        _error "快照未通过 Xray 配置校验，已取消恢复。"
        return 1
    fi
    [ -f "${CONFIG_FILE}.restore.tmp" ] && mv -f "${CONFIG_FILE}.restore.tmp" "$CONFIG_FILE"
    [ -f "${SINGBOX_DIR}/relay.json.restore.tmp" ] && mv -f "${SINGBOX_DIR}/relay.json.restore.tmp" "${SINGBOX_DIR}/relay.json"
    [ -f "${METADATA_FILE}.restore.tmp" ] && mv -f "${METADATA_FILE}.restore.tmp" "$METADATA_FILE"
    [ -f "${CLASH_YAML_FILE}.restore.tmp" ] && mv -f "${CLASH_YAML_FILE}.restore.tmp" "$CLASH_YAML_FILE"
    if [ -f "$snapshot/xray/config.json" ]; then
        mkdir -p /usr/local/etc/xray
        cp -a "$snapshot/xray/config.json" /usr/local/etc/xray/config.json
    fi
    _manage_service restart
    _success "配置已恢复并重启 Sing-box；恢复前的配置已自动保存。"
}

_rotate_log_file() {
    local log_file="$1" base size archive
    [ -f "$log_file" ] || return 0
    size=$(wc -c < "$log_file" 2>/dev/null || echo 0)
    [ "$size" -ge "$LOG_ROTATE_MAX_BYTES" ] || return 0
    mkdir -p "$LOG_ARCHIVE_DIR"
    base=$(basename "$log_file")
    archive="${LOG_ARCHIVE_DIR}/${base}.$(date +%Y%m%d_%H%M%S)"
    cp "$log_file" "$archive" && : > "$log_file"
    command -v gzip >/dev/null 2>&1 && gzip -f "$archive"
}
...
_maintain_logs() {
    ...
    find "$LOG_ARCHIVE_DIR" -type f -mtime +"$LOG_RETENTION_DAYS" -delete 2>/dev/null || true
...
_operations_menu() {
    ...
    while true; do
        clear
        echo -e "${CYAN}【运维工具】${NC}"
        echo -e "  ${GREEN}[1]${NC} VPS 与服务状态面板"
        echo -e "  ${GREEN}[2]${NC} 创建完整配置快照"
        echo -e "  ${GREEN}[3]${NC} 从配置快照恢复"
        echo -e "  ${GREEN}[4]${NC} 立即轮转日志并清理过期归档"
        echo -e "  ${YELLOW}[0]${NC} 返回主菜单"
        read -r -p "请输入选项 [0-4]: " choice
        case "$choice" in
            1) _show_operations_status ;;
            2) _create_config_backup manual; read -n 1 -s -r -p "按任意键继续..." ;;
            3) _restore_config_backup; read -n 1 -s -r -p "按任意键继续..." ;;
            4) _maintain_logs; _success "日志维护完成：超过 10MB 的脚本日志已归档，保留 ${LOG_RETENTION_DAYS} 天。"; read -n 1 -s -r -p "按任意键继续..." ;;
            0) return ;;
            *) _error "无效输入。"; sleep 1 ;;
        esac
    done
...
_main_menu() {
    while true; do
        ...
        echo -e "    ${GREEN}[20]${NC} VPS 网络优化      ${GREEN}[21]${NC} 运维工具"
        echo ""
